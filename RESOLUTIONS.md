...
- **Finish aligning the install policy.** `npmMinimalAgeGate` was moved from `0` to `7d` on 2026-08-05 to match the application that treats this repository as its reference — a baseline looser than the apps copying it is the divergence a baseline exists to prevent. Turning it on cost nothing: the lockfile came back byte-identical, because no installed release was younger than the window. Two settings are still looser here than in that application, and neither has a recorded reason: `enableScripts: true`, and the absence of `enableHardenedMode` and `checksumBehavior: throw`. Both are worth an explicit answer rather than being left at whatever they happen to be.
